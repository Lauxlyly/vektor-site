...
function buildEmail(strategy, report, auditId) {
  ...
  // Edge-source pill: green ONLY for the 4 real sources; unknown/malformed -> gray, never green.
  const ea = report.edge_analysis || null;
  const _es = ea ? String(ea.edge_source || '').toLowerCase() : '';
  const _strong = ['speed', 'information', 'structure', 'risk-premium'].includes(_es);
  const _weak = ['prediction-only', 'none-identifiable'].includes(_es);
  const edgeLabel = (_strong || _weak) ? _es.replace(/-/g, ' ') : 'unclassified';
  const edgePill = _strong ? 'background:rgba(74,222,128,.13);color:#4ade80;'
    : _weak ? 'background:rgba(239,68,68,.15);color:#f87171;'
...
<div style="max-width:660px;margin:0 auto;padding:32px 20px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
  ...
  ${report.edge_analysis ? `<div style="background:#0a0a16;border:1px solid rgba(124,58,237,.2);border-radius:12px;padding:20px 24px;margin-bottom:24px;">
    <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:12px;">
      <span style="font-family:monospace;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#7c3aed;">Edge Source &amp; Counterparty</span>
      <span style="font-family:monospace;font-weight:800;padding:3px 11px;border-radius:100px;font-size:12px;${edgePill}">${esc(edgeLabel)}</span>
    </div>
    <div style="font-size:13px;color:#cbd5e1;line-height:1.7;"><span style="color:#64748b;">Who loses to you &amp; why they can't stop: </span>${esc(report.edge_analysis.counterparty||'—')}</div>
    <div style="font-size:13px;color:#cbd5e1;line-height:1.7;margin-top:8px;">${esc(report.edge_analysis.assessment||'')}</div>
  </div>` : ''}
  ...
  <div style="background:#0a0a16;border:1px solid rgba(124,58,237,.2);border-radius:12px;overflow:hidden;margin-bottom:24px;">
    <div style="padding:16px 20px;background:rgba(124,58,237,.07);border-bottom:1px solid rgba(124,58,237,.12);">
      <span style="font-family:monospace;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#7c3aed;">Falsification Review · 10 Failure-Mode Checks</span>
